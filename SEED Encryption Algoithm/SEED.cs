...
namespace SEED_Encryption_Algoithm
{
    public class SEED
    {
        ...
        public static UInt32[] getParts(byte[] text)
        {
            byte[] l0 = new byte[4];
            byte[] l1 = new byte[4];
            byte[] r0 = new byte[4];
            ...
            {
                l0[i] = text[i];
                l1[i] = text[text.Length - 12 + i];
                r0[i] = text[text.Length - 8 + i];
                r1[i] = text[text.Length - 4 + i];
            }
            Array.Reverse(l0);
            Array.Reverse(l1);
            Array.Reverse(r0);
            Array.Reverse(r1);
            UInt32[] parts = new UInt32[4];
            parts[0] = BitConverter.ToUInt32(l0, 0);
            parts[1] = BitConverter.ToUInt32(l1, 0);
            parts[2] = BitConverter.ToUInt32(r0, 0);
            parts[3] = BitConverter.ToUInt32(r1, 0);
            return parts;
        }

        public static byte[] assembleParts(byte[] p1, byte[] p2, byte[] p3, byte[] p4)
        {
            byte[] res = new byte[p1.Length + p2.Length + p3.Length + p4.Length];
            p1.CopyTo(res, 0);
            p2.CopyTo(res, p1.Length);
            p3.CopyTo(res, p1.Length + p2.Length);
            p4.CopyTo(res, p1.Length + p2.Length + p3.Length);
            return res;
        }
